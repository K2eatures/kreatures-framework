...
import com.github.kreatures.gui.util.tree.UserObjectWrapperController;
import com.github.kreatures.gui.util.tree.UserObjectWrapperTreeCellRenderer;
...
public class ReportTreeView 
	...
	private static class AgentUserObjectWrapper extends DefaultUserObjectWrapper {
		...
		@Override
		public Icon getIcon() {
			return KReaturesWindow.get().getIcons().get("agent");
		}
	...
	private static class EntryUserObjectWrapper extends DefaultUserObjectWrapper {
		...
		public Icon getIcon() {
			...
			if(entry.getAttachment() == null) {
				return KReaturesWindow.get().getIcons().get("report");
			} else {
				return KReaturesWindow.get().getIcons().get("report_attachment");
			}
		...
		public void onActivated() {
			...
			if(entry.getAttachment() != null) {
				...
				KReaturesWindow.get().openView(view);
