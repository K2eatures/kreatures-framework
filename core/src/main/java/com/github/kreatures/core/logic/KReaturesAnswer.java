...
/**
 * An answer in the KReatures framework. Extends the GenericAnswer by an extended 
 ...
 * like "isDeveloper(X)"
 * 
 * @see kreatures.core.logic.AnswerValue
 * @author Tim Janus
 * @author Daniel Dilger
 */
public class KReaturesAnswer {
	 ...
	 */
	public KReaturesAnswer(FolFormula query) {
	 ...
	 */
	public KReaturesAnswer(FolFormula query, AnswerValue av) {
		setAnswer(av);
		this.query = query;
	}
	
	public KReaturesAnswer(FolFormula query, FolFormula answer) {
	...
	public KReaturesAnswer(FolFormula query, Set<FolFormula> formulas) {
		setAnswer(formulas);
		this.query = query;
	}
	...
	public boolean equals(Object other) {
		if(!(other instanceof KReaturesAnswer))
			return false;
		
		KReaturesAnswer aa = (KReaturesAnswer)other;
