...
public class AspReasoner extends BaseReasoner {
	...
	protected Pair<Set<FolFormula>, KReaturesAnswer> queryImpl(ReasonerParameter params) {		
		...
		if(query.isGround()) {
			...
			return new Pair<>(answers, new KReaturesAnswer(query, av));
		} else {
			...
			return new Pair<>(answers, new KReaturesAnswer(query, realAnswers));
		}
	}
	...
	private List<AnswerSet> runSolver(AspBeliefbase bb) throws SolverException {
		...
		return s.computeModels(bb.getProgram(), 1000);
	}
