...
public class SwarmSubgoalGenerationOperator extends BaseSubgoalGenerationOperator{
	/** reference to the logback logger instance */
	private Logger LOG = LoggerFactory.getLogger(SwarmSubgoalGenerationOperator.class);
	
	@Override
	protected Boolean processImpl(PlanParameter pParameters) {
		LOG.info("Run Default-Subgoal-Generation");
		Agent ag = pParameters.getActualPlan().getAgent();

		boolean reval = processPersuadeOtherAgentsDesires(pParameters, ag);

		Desires des = ag.getComponent(Desires.class);
		if(des != null) {
			Set<Desire> currentDesires;
			currentDesires = des.getDesiresByPredicate(SwarmGenerateOptionsOperator.prepareQueryProcessing);
			for(Desire d : currentDesires) {
				if(pParameters.getActualPlan().countPlansFor(d) > 0)
					continue;
				reval = reval || answerQuery(d, pParameters, ag);
			}
			
			currentDesires = des.getDesiresByPredicate(SwarmGenerateOptionsOperator.prepareRevisionRequestProcessing);
			for(Desire d : currentDesires) {
				if(pParameters.getActualPlan().countPlansFor(d) > 0)
					continue;
				reval = reval || informProcessing(d, pParameters, ag);
			}
			
			currentDesires = des.getDesiresByPredicate(SwarmGenerateOptionsOperator.prepareScriptingProcessing);
			pParameters.report("desires:"+des.getDesires());
			for(Desire d : currentDesires){
				if(pParameters.getActualPlan().countPlansFor(d) > 0)
					continue;
				reval = processScripting(d, pParameters, ag);
			}
		}
		
		if(!reval)
			pParameters.report("No new subgoal generated.");
		return reval;
	}
	...
	protected Boolean answerQuery(Desire des, PlanParameter pp, Agent ag) {
		...
		KReaturesAnswer aa = bb.reason(q.getQuestion());
